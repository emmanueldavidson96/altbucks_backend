...
export const getRewards = async (userId: string) => {
  ...
  // Calculate rewards based on active or completed referrals
  const rewards = referral.activities.filter(
    (activity) => activity.status === "active" || activity.status === "completed"
  );

  return {
    totalReferrals: referral.activities.length,
    activeReferrals: rewards.length,
    rewards,
  };
};
